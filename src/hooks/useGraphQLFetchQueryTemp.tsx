...
const useGraphQLFetchQueryTemp = <T,>() => {
  ...
  const performQuery = async (
    ...
    const query = `
    query ${queryName}($limit: Int, $page: Int${Object.keys(restVariables).length > 0
        ? ', ' +
      ...
      ${queryName}(limit: $limit, page: $page${Object.keys(restVariables).length > 0
        ? ', ' +
        Object.keys(restVariables)
  ...
  `
    ...
    try {
      ...
      if (queryResult && queryResult.projects) {
        ...
        // console.log(`${queryName} fetched successfully:`, queryResult.projects)
      } else {
...
export default useGraphQLFetchQueryTemp
